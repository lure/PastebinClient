...
uses
  ...
  FMX.ActnList, FMX.Types, FMX.Objects, FMX.StdCtrls, FMX.Controls;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if not FCustomIcon.BitmapHiRes.IsEmpty then
          ImageGenSrv.GenerateTabIcon(FCustomIcon.BitmapHiRes, FIconObject.BitmapHiRes, IsSelected, True)
        else
          ImageGenSrv.GenerateTabIcon(FCustomIcon.Bitmap, FIconObject.BitmapHiRes, IsSelected, True);
      end;
    ...
    begin
      ...
      if not FCustomIcon.BitmapHiRes.IsEmpty then
        FIconObject.BitmapHiRes := FCustomIcon.BitmapHiRes
      else
        FIconObject.BitmapHiRes := FCustomIcon.Bitmap;
    end;
  end;
end;
      ...
      begin
        ...
        try
          ...
          begin
            ...
            LocalAnimateInt(Layout1, 'Position.X', Round(P.X - LayoutRect.Width), Duration, TAnimationType.atIn, TInterpolationType.itCubic);
            P := Tab2.AbsoluteToLocal(LayoutPos);
            Layout2.SetBounds(P.X + LayoutRect.Width, P.Y, LayoutRect.Width, LayoutRect.Height);
            LocalAnimateIntWait(Layout2, 'Position.X', Round(P.X), Duration, TAnimationType.atIn, TInterpolationType.itCubic);
          end
          else
          begin
            P := Tab1.AbsoluteToLocal(LayoutPos);
